...
type TreatmentPlanHandler struct{ service service.TreatmentPlanService }

func NewTreatmentPlanHandler(s service.TreatmentPlanService) *TreatmentPlanHandler {
	return &TreatmentPlanHandler{service: s}
}

func (h *TreatmentPlanHandler) Register(group *gin.RouterGroup) {
...
func (h *TreatmentPlanHandler) list(c *gin.Context) {
	query := bindPage(c)
...
func (h *TreatmentPlanHandler) get(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *TreatmentPlanHandler) create(c *gin.Context) {
	var input dto.CreateTreatmentPlan
...
func (h *TreatmentPlanHandler) update(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.UpdateTreatmentPlan
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
...
func (h *TreatmentPlanHandler) transition(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *TreatmentPlanHandler) remove(c *gin.Context) {
