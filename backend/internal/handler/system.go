...
type SystemHandler struct {
	...
	treatmentPlan service.TreatmentPlanService
	materialTest  service.MaterialTestService
	stageApproval service.StageApprovalService
	db            *gorm.DB
	redis         *redis.Client
}

func NewSystemHandler(security service.SecurityService, artifact service.ArtifactService, treatmentPlan service.TreatmentPlanService, materialTest service.MaterialTestService, stageApproval service.StageApprovalService, db *gorm.DB, redisClient *redis.Client) *SystemHandler {
	return &SystemHandler{security: security, artifact: artifact, treatmentPlan: treatmentPlan, materialTest: materialTest, stageApproval: stageApproval, db: db, redis: redisClient}
}
...
func (h *SystemHandler) Overview(c *gin.Context) {
	...
	artifactCounts, err := h.artifact.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["artifacts"] = artifactCounts

	treatmentPlanCounts, err := h.treatmentPlan.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["plans"] = treatmentPlanCounts

	materialTestCounts, err := h.materialTest.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["tests"] = materialTestCounts

	stageApprovalCounts, err := h.stageApproval.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["approvals"] = stageApprovalCounts
