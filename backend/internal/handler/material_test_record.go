...
type MaterialTestHandler struct{ service service.MaterialTestService }

func NewMaterialTestHandler(s service.MaterialTestService) *MaterialTestHandler {
	return &MaterialTestHandler{service: s}
}

func (h *MaterialTestHandler) Register(group *gin.RouterGroup) {
	resource := group.Group("/tests")
	resource.GET("", h.list)
...
func (h *MaterialTestHandler) list(c *gin.Context) {
	query := bindPage(c)
...
func (h *MaterialTestHandler) get(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *MaterialTestHandler) create(c *gin.Context) {
	var input dto.CreateMaterialTest
...
func (h *MaterialTestHandler) update(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.UpdateMaterialTest
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
...
func (h *MaterialTestHandler) transition(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *MaterialTestHandler) remove(c *gin.Context) {
