...
type ArtifactHandler struct{ service service.ArtifactService }

func NewArtifactHandler(s service.ArtifactService) *ArtifactHandler {
	return &ArtifactHandler{service: s}
}

func (h *ArtifactHandler) Register(group *gin.RouterGroup) {
	resource := group.Group("/artifacts")
	resource.GET("", h.list)
...
func (h *ArtifactHandler) transition(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
