...
type StageApprovalHandler struct{ service service.StageApprovalService }

func NewStageApprovalHandler(s service.StageApprovalService) *StageApprovalHandler {
	return &StageApprovalHandler{service: s}
}

func (h *StageApprovalHandler) Register(group *gin.RouterGroup) {
	resource := group.Group("/approvals")
	resource.GET("", h.list)
	...
	resource.POST("/:id/corrections", middleware.RequireMinimumRole("operator"), h.submitCorrection)
	resource.DELETE("/:id", middleware.RequireRoles("admin"), h.remove)
}

func (h *StageApprovalHandler) list(c *gin.Context) {
	query := bindPage(c)
...
func (h *StageApprovalHandler) get(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *StageApprovalHandler) create(c *gin.Context) {
	var input dto.CreateStageApproval
...
func (h *StageApprovalHandler) update(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.UpdateStageApproval
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
...
func (h *StageApprovalHandler) transition(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *StageApprovalHandler) remove(c *gin.Context) {
